...
var filterPatterns = (command) => {
  ...
  let patternIndexes = getSettings().patterns.map((pattern, patternIndex) => {
    return { pattern, patternIndex };
  }).filter((p) => p.pattern.rules.length > 0 && p.pattern.rules.every((rule) => rule.from !== "")).map((p) => p.patternIndex);
